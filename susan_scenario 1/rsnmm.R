...
cluster_level_treatment = function(group,T){
  treatment_total = sample(c(0,1), group[["#groups"]] *T, replace = TRUE)
  treatment_matrix = matrix(treatment_total,nrow = group[["#groups"]])
  A_matrix = treatment_matrix[group[["group_id"]],]
  c(t(A_matrix))
}
...
rsnmm.R <- function(n, tmax, group_ls, control, ...) {
  ...
  d <- rsnmm(
    ...
    a = cluster_level_treatment(group,tmax),
